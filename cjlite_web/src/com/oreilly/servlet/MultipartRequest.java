...
/**
 * A utility class to handle <code>multipart/form-data</code> requests, the kind of requests that support file uploads.
 * This class emulates the interface of <code>HttpServletRequest</code>, making it familiar to use. It uses a "push"
 * model where any incoming files are read and saved directly to disk in the constructor. If you wish to have more
 * flexibility, e.g. write the files to a database, use the "pull" model <code>MultipartParser</code> instead.
 * <p>
 * This class can receive arbitrarily large files (up to an artificial limit you can set), and fairly efficiently too.
 * It cannot handle nested data (multipart content within multipart content). It <b>can</b> now with the latest release
 * handle internationalized content (such as non Latin-1 filenames).
 * <p>
 * To avoid collisions and have fine control over file placement, there's a constructor variety that takes a pluggable
 * FileRenamePolicy implementation. A particular policy can choose to rename or change the location of the file before
 * it's written.
 * <p>
 ...
 * @version 1.9, 2002/04/30, added support for file renaming, thanks to Changshin Lee<br>
 * @version 1.8, 2002/04/30, added support for internationalization, thanks to Changshin Lee<br>
 * @version 1.7, 2001/02/07, made fields protected to increase user flexibility<br>
 * @version 1.6, 2000/07/21, redid internals to use MultipartParser, thanks to Geoff Soutter<br>
 * @version 1.5, 2000/02/04, added auto MacBinary decoding for IE on Mac<br>
 * @version 1.4, 2000/01/05, added getParameterValues(), WebSphere 2.x getContentType() workaround, stopped writing
 *          empty "unknown" file<br>
...
public class MultipartRequest {
	...
	/**
	 * Constructs a new MultipartRequest to handle the specified request, saving any uploaded files to the given
	 * directory, and limiting the upload size to 1 Megabyte. If the content is too large, an IOException is thrown.
	 * This constructor actually parses the <tt>multipart/form-data</tt> and throws an IOException if there's any
	 ...
	 *                if the uploaded content is larger than 1 Megabyte or there's a problem reading or parsing the
	 *                request.
	 */
	...
	/**
	 * Constructs a new MultipartRequest to handle the specified request, saving any uploaded files to the given
	 * directory, and limiting the upload size to the specified length. If the content is too large, an IOException is
	 * thrown. This constructor actually parses the <tt>multipart/form-data</tt> and throws an IOException if there's
	 * any problem reading or parsing the request.
	 * 
	 * @param request
	 *            the servlet request.
	 * @param saveDirectory
	 ...
	 * @exception IOException
	 *                if the uploaded content is larger than <tt>maxPostSize</tt> or there's a problem reading or
	 *                parsing the request.
	 */
	 ...
	 * any problem reading or parsing the request.
	 * 
	 * To avoid file collisions, this constructor takes an implementation of the FileRenamePolicy interface to allow a
	 * pluggable rename policy.
	 * 
	 * @param request
	 *            the servlet request.
	 * @param saveDirectory
		...
		while ((part = parser.readNextPart()) != null) {
			...
			if (part.isParam()) {
				...
				if (fileName != null) {
					...
					files.put(
							name,
							new UploadedFile(dir.toString(), filePart.getFileName(), fileName, filePart
									.getContentType(),null));
				} else {
					// The field did not contain a file
					files.put(name, new UploadedFile(null, null, null, null,null));
				}
			}
		}
	}
	...
	/**
	 * Constructor with an old signature, kept for backward compatibility. Without this constructor, a servlet compiled
	 * against a previous version of this class (pre 1.4) would have to be recompiled to link with this version. This
	 * constructor supports the linking via the old signature. Callers must simply be careful to pass in an
	 * HttpServletRequest.
	 * 
	 */
	...
	/**
	 * Returns the names of all the uploaded files as an Enumeration of Strings. It returns an empty Enumeration if
	 * there are no file input fields on the form. Any file input field that's left empty will result in a FilePart with
	 * null contents. Each file name is the name specified by the form, not by the user.
	 * 
	...
	/**
	 * Returns the filesystem name of the specified file, or null if the file was not included in the upload. A
	 * filesystem name is the name specified by the user. It is also the name under which the file is actually saved.
	 * 
	 * @param name
	...
	/**
	 * Returns the original filesystem name of the specified file (before any renaming policy was applied), or null if
	 * the file was not included in the upload. A filesystem name is the name specified by the user.
	 * 
	 * @param name
	 *            the html page's file parameter name.
	...
	/**
	 * Returns the content type of the specified file (as supplied by the client browser), or null if the file was not
	...
	/**
	 * Returns a File object for the specified file saved on the server's filesystem, or null if the file was not
	 * included in the upload.
	 * 
	 * @param name
	 *            the html page's file parameter name.
