...
/**
 * A <code>PartInputStream</code> filters a <code>ServletInputStream</code>, providing access to a single MIME part
 * contained with in which ends with the boundary specified. It uses buffering to provide maximum performance.
 * <p>
 * Note the <code>readLine</code> method of <code>ServletInputStream</code> has the annoying habit of adding a \r\n to
 * the end of the last line. Since we want a byte-for-byte transfer, we have to cut those chars. This means that we must
 * always maintain at least 2 characters in our buffer to allow us to trim when necessary.
 * 
 * @author Geoff Soutter
 * @author Jason Hunter
 * @version 1.4, 2002/11/01, fix for "unexpected end of part" caused by boundary newlines split across buffers
