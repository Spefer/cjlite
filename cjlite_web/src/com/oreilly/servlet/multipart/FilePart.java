...
public class FilePart extends Part {
	 ...
	 * @param filePath
	 *            the file system path of the file provided in the MIME header (as specified in disposition info).
	 * 
	...
	/**
	 * Returns the name that the file was stored with on the remote system, or <code>null</code> if the user didn't
	 * enter a file to be uploaded. Note: this is not the same as the name of the form parameter used to transmit the
	 * file; that is available from the <code>getName</code> method. Further note: if file rename logic is in effect,
	 * the file name can change during the writeTo() method when there's a collision with another file of the same name
	...
	/**
	 * Returns the full path and name of the file on the remote system, or <code>null</code> if the user didn't enter a
	 * file to be uploaded. If path information was not supplied by the remote system, this method will return the same
	...
	/**
	 * Returns an input stream which contains the contents of the file supplied. If the user didn't enter a file to
	 * upload there will be <code>0</code> bytes in the input stream. It's important to read the contents of the
	 * InputStream immediately and in full before proceeding to process the next part. The contents will otherwise be
	 * lost on moving to the next part.
	 * 
	...
	/**
	 * Write this file part to a file or directory. If the user supplied a file, we write it to that file, and if they
	 * supplied a directory, we write it to that directory with the filename that accompanied it. If this part doesn't
	...
	public long writeTo(File fileOrDirectory) throws IOException {
		...
		try {
			...
			if (fileName != null) {
				...
				logger.debug("start to write file:{0}", file.getAbsolutePath());
				written = write(fileOut);
				logger.debug("write file {0}, length:{0}", file.getAbsolutePath(),written);
			}
		} finally {
			if (fileOut != null)
				fileOut.close();
