 ...
 * This class uses a "pull" model where the reading of incoming files and parameters is controlled by the client code,
 * which allows incoming files to be stored into any <code>OutputStream</code>. If you wish to use an API which
 * resembles <code>HttpServletRequest</code>, use the "push" model <code>MultipartRequest</code> instead. It's an
 * easy-to-use wrapper around this class.
 * <p>
 ...
 * <p>
 * It also optionally includes enhanced buffering and Content-Length limitation. Buffering is only required if your
 * servlet container is poorly implemented (many are, including Tomcat 3.2), but it is generally recommended because it
 * will make a slow servlet container a lot faster, and will only make a fast servlet container a little slower.
 * Content-Length limiting is usually only required if you find that your servlet is hanging trying to read the input
 * stram from the POST, and it is similarly recommended because it only has a minimal impact on performance.
 * <p>
 ...
 * @version 1.11, 2002/11/01, added constructor that takes an encoding, to make sure chars are always read correctly
 * @version 1.10, 2002/11/01, added support for a preamble before the first boundary marker
 ...
 * @version 1.6, 2002/04/30, added better internationalization support, thanks to Changshin Lee
 ...
 * @version 1.3, 2001/01/22, added support for boundaries surrounded by quotes and content-disposition after
 *          content-type, thanks to Scott Stark
...
public class MultipartParser {
	...
	/**
	 * Creates a <code>MultipartParser</code> from the specified request, which limits the upload size to the specified
	 * length, buffers for performance and prevent attempts to read past the amount specified by the Content-Length.
	 * 
	 * @param req
	 *            the servlet request.
	 * @param maxSize
	...
	/**
	 * Creates a <code>MultipartParser</code> from the specified request, which limits the upload size to the specified
	 * length, and optionally buffers for performance and prevents attempts to read past the amount specified by the
	 * Content-Length.
	 * 
	 * @param req
	 *            the servlet request.
	 ...
	 *            the maximum size of the POST content.
	 * @param buffer
	 *            whether to do internal buffering or let the server buffer, useful for servers that don't buffer
	 * @param limitLength
	 *            boolean flag to indicate if we need to filter the request's input stream to prevent trying to read
	...
	/**
	 * Sets the encoding used to parse from here onward. The default is ISO-8859-1. Encodings are actually best passed
	 * into the contructor, so even the initial line reads are correct.
	 * 
	 * @param encoding
